...
# Start the CDH Cluster Services using REST API of Cloudera Manager
/usr/bin/curl -X POST -u "$username:$password" -k -i "$protocol://$ipaddress:$port/api/v1/clusters/$clustername/commands/start"
