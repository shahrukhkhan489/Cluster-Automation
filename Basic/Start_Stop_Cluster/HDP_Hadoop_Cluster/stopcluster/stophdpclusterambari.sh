...
xargs -a services.list -i{} echo /usr/bin/curl -u $username:$password -k -i -H \'X-Requested-By: ambari\' -X PUT -d \'{\"RequestInfo\": {\"context\" :\"Stop {} via REST\"}, \"Body\": {\"ServiceInfo\": {\"state\": \"INSTALLED\"}}}\' $protocol://$ipaddress:$port/api/v1/clusters/$clustername/services/{} > stopscript.sh


#execute stopscript.sh
sh stopscript.sh
